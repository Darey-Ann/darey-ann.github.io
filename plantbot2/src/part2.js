...
uniform int  uFlag;              // 0 std, 1 solar panel, 2 sky, 3 wood, 4 wall, 5 shadow blob,
                                 // 6 LED rim, 7 fenestrated blade, 8 floorboards
uniform vec3 uCamPos;
uniform vec3 uSunDir, uSunColor;
uniform vec3 uSkyCol, uGndCol;
uniform float uAmb;
uniform vec3 uPL0, uPL1, uPL2;       // point light positions
uniform vec3 uPC0, uPC1, uPC2;       // colors * intensity
...
void main(){
  ...
  if (uFlag == 7) {                                  // monstera fenestration
    // punch the blade through with oval holes, mirrored either side of the midrib.
    // u runs base->tip, r is 0 at the midrib and 1 at the margin.
    float u = vUV.x, r = abs(vUV.y - 0.5) * 2.0;
    float hole = 0.0;
    hole = max(hole, 1.0 - length(vec2((u - 0.375) / 0.052, (r - 0.34) / 0.20)));
    hole = max(hole, 1.0 - length(vec2((u - 0.545) / 0.046, (r - 0.32) / 0.19)));
    hole = max(hole, 1.0 - length(vec2((u - 0.715) / 0.038, (r - 0.30) / 0.17)));
    if (hole > 0.0) discard;
  }
  if (uFlag == 8) {                                  // floorboards
    float seam = smoothstep(0.006, 0.030, abs(fract(vWorld.x * 3.2) - 0.5) * 0.62);
    float g = sin(vWorld.z * 46.0 + sin(vWorld.x * 5.0) * 3.1) * 0.5 + 0.5;
    albedo *= (0.84 + 0.16 * g) * mix(0.58, 1.0, seam);
  }
  ...
  // fresnel sheen keeps the molded plastic reading as a surface at grazing angles
...
uniform float uBloomStr, uExposure, uGrain;
...
void main(){
  ...
  // Static dither. This breaks up 8-bit banding on the smooth gradients (wall, desk
  // falloff). It must NOT vary with time - animated noise reads as a fluorescent strobe.
